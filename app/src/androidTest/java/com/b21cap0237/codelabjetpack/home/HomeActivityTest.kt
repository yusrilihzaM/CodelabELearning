package com.b21cap0237.codelabjetpack.home
...
class HomeActivityTest{
    ...
    fun loadDetailCourse() {
        //item index 0 di click
        onView(withId(R.id.rv_academy)).perform(RecyclerViewActions.actionOnItemAtPosition<RecyclerView.ViewHolder>(0, click()))
        onView(withId(R.id.text_title)).check(matches(isDisplayed()))// cek judul
        onView(withId(R.id.text_title)).check(matches(withText(dummyCourse[0].title)))// compare judul
        onView(withId(R.id.text_date)).check(matches(isDisplayed()))// cek tanggal
        onView(withId(R.id.text_date)).check(matches(withText("Deadline ${dummyCourse[0].deadline}")))//compare tanggal
    }
    @Test
    fun loadModule() {
        onView(withId(R.id.rv_academy)).perform(RecyclerViewActions.actionOnItemAtPosition<RecyclerView.ViewHolder>(0, click()))
